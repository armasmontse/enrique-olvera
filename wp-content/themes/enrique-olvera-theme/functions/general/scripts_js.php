...
add_action( 'admin_enqueue_scripts', 'cltvo_admin_js',( PHP_INT_MAX - 2 ) ); // incluye el admin-functions.js. Descomentar para tener JS en admin (no olvidar crear el file [admin-functions.js])
...
function cltvo_js(){
	...
	wp_register_script( 'cltvo_functions_js', JSPATH.'functions.js', array(
		...
		/*'cltvo_scrollIt_js', 'cltvo_swiper_js','cltvo_validate_js', 'cltvo_validate_espanol_js'*/
	), false, true );


	//CDN jquery_modal
  	wp_register_script( 'jquery_modal', '//cdnjs.cloudflare.com/ajax/libs/jquery-modal/0.9.1/jquery.modal.min.js', array('jquery'), false, true );
...
function cltvo_admin_js(){
	...
	wp_enqueue_script('jquery');
	wp_enqueue_script('cltvo_admin_functions_js', null, null, true );
}
